...
import {
  LayoutDashboard, AlertTriangle, Cloud, Tent, Heart, LogOut, Phone,
  User, Menu, X
} from "lucide-react";

const navItems = [
  { label: "Dashboard", icon: LayoutDashboard, path: "/dashboard" },
  { label: "Alert Areas", icon: AlertTriangle, path: "/dashboard/alerts" },
  { label: "Weather Monitoring", icon: Cloud, path: "/dashboard/weather" },
  { label: "Relief Camps", icon: Tent, path: "/dashboard/relief" },
  { label: "Medical Support", icon: Heart, path: "/dashboard/medical" },
];
      ...
      <div className="p-5 border-b border-sidebar-muted/30">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-sidebar-fg/10 flex items-center justify-center">
            <span className="text-sidebar-fg font-bold">R</span>
          </div>
          <span className="text-lg font-bold text-sidebar-fg">ResQHub</span>
        </div>
      </div>
